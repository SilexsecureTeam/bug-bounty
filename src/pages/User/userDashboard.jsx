...
export default function UserDashboard() {
  ...
  const isGroup = currentUser?.user_type === "group";
  ...
  const activeReportsCount = reportStats
    ? (reportStats.reportnew || 0) +
      (reportStats.reportreview || 0) +
      (reportStats.reportfix || 0)
    : 0;
  ...
  const stats = {
    ...
    reputation: reportStats?.reportpoint
      ? Number(reportStats.reportpoint).toLocaleString()
      : 0,
    balance: reportStats?.balance
      ? Number(reportStats.balance).toLocaleString()
      : "0.00",
        ...
        <main className="flex-1 w-full sm:ml-64 p-6 sm:p-8 overflow-y-auto">
          ...
          <div className="mb-6 grid w-full grid-cols-1 gap-4 sm:grid-cols-4">
            <div className="rounded-xl border border-[#1b1f24] bg-[#0c0f12] p-4">
              <div className="text-xs font-semibold text-[#9aa4b0]">
                Your rank
              </div>
            ...
            <div className="flex flex-col gap-4">
              <div>
                <h2 className="text-xl font-bold">
                  Join the Defcomm Community
                </h2>
                <p className="mt-1 text-sm text-[#b7c2aa]">
                  Collaborate, share, and secure the future together
                </p>
              </div>
              <div className="mt-2 flex items-center gap-3">
                <button className="rounded-full bg-[#9fc24d] px-4 py-2 font-semibold text-[#071000]">
                  Join Defcomm Channel
                </button>
              </div>
            </div>
          </div>
          ...
          {isGroup && (
            <div className="flex w-full items-end justify-between flex-wrap gap-4">
              <button className="rounded-md bg-[#97c44a] px-3 py-2 mb-4 font-semibold text-[#071000]">
                <Link to="/group/members-list">List of Members</Link>{" "}
              </button>
              <button className="rounded-md bg-[#97c44a] px-3 py-2 mb-4 font-semibold text-[#071000]">
                <Link to="/group/invite-members">Invite New Members</Link>{" "}
              </button>
            </div>
          )}
          {/* public link card */}
          <div className="mb-6 rounded-2xl border border-[#16171b] bg-[#07080b] p-6">
            <div className="flex flex-col">
              <div className="text-sm font-semibold text-[#cfe3b1]">
                Your public link
              </div>
            ...
            <p className="mt-3 text-xs text-[#98a1ad]">
              If you want to change address, go to{" "}
              <span className="text-[#9fc24d]">settings</span> and change your
              nickname.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-dashed border-[#2a2d31] bg-[#060708] p-8 text-center text-[#aeb6bf]">
            ...
            <p className="mt-3">
              Browse Defcomm's live bounty programs, hunt for vulnerabilities,
              and submit
            </p>
            <div className="mt-6 flex items-center justify-center gap-3">
              <button className="rounded-md border border-[#2b2f33] px-4 py-2 text-sm">
                Browse programs
              </button>
              <button className="rounded-md bg-[#9fc24d] px-4 py-2 text-sm font-semibold text-[#071000]">
